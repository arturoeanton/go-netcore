...
| [HCL.md](HCL.md) | The HCL milestone: `hashicorp/hcl/v2` running on the CLR and packaged as the `Hcl.GoCLR` C# NuGet — byte-exact parsing, the reflect/`big.Accuracy` fixes, and the honest limits |
| [OPA.md](OPA.md) | The OPA milestone: `open-policy-agent/opa` running on the CLR and packaged as the `Opa.GoCLR` C# NuGet — byte-exact Rego evaluation, the named-scalar/generic-identity fixes, and the honest limits |
