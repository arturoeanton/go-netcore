...
| Generics: same- AND cross-package instantiation + explicit type args (Fn[T]), incl. an instantiation used as a value (cmp.Compare[int] passed to slices.SortFunc) and method dispatch through a generic interface (Container[T]) | ✅ | L | ✅ |
...
## 4. Stdlib overlay (C# shim mechanism live; 418 conformance fixtures byte-exact; P0/P1/P2/P3/P4 hardened, typed-box + goja + Gin + Echo running)
...
| 418 conformance fixtures, all byte-exact vs `go run` | ✅ | M | ✅ |
